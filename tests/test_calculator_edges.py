...
def test_car_value_calculator_monthly_depr_and_total_cost():
    ...
    calc = CarValueCalculator(series, number_of_years=2, monthly_maintenance=50.0, purchase_year_index=1, loan_value=0.0)
    monthly_depr = calc.monthly_depreciation()
    assert pytest.approx(monthly_depr, rel=1e-6) == (28000 - 24000) / (2 * 12.0)
    # TCO = ((PMT*Lt) + (P-L) + (M*Ot) - Vf) / Ot
    # If no loan: (0 + P + M*Ot - Vf) / Ot = (P-Vf)/Ot + M = monthly_depr + M
    total = calc.monthly_total_cost(loan_monthly=0.0, loan_years=0)
    assert pytest.approx(total, rel=1e-6) == monthly_depr + 50.0
